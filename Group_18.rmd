...
poisson_model_full <- glm(data = animals, time_at_shelter ~ ., family = poisson)
...
$$I_{\mbox{OwnerSurrender}}(j) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~jth~\mbox{observation is OwnerSurrender},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$

$$I_{\mbox{Stray}}(j) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~jth~\mbox{observation is Stray},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$

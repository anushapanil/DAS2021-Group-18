...
```{r outliers}
plot(animals$time_at_shelter, as.factor(animals$animal_type), main = "Plot of Time at shelter vs Animal Type", xlab = "Time at shelter", ylab = "Animal type", col = "darkred")
...
```{r model_comparisons}
c(q_poisson_model$deviance, q_poisson_model$deviance)
...
$$ \widehat{time~at~shelter}(i) ~=~ 44.812 ~+~ 0.804~\cdot~ I_{ScanNoChip}~ (i) ~+~ 0.448 ~\cdot~ I_{UnableToScan}~ (i) ~+~
0.630 ~\cdot~ I_{Died}~ (i) ~+~ \\
0.524 ~\cdot~ I_{Euthanized}~ (i) ~+~ 0.555 ~\cdot~ I_{Foster}~ (i) ~+~ 0.158 ~\cdot~ I_{ReturnedToOwner}~ (i) ~+~ 0.165 ~\cdot~ I_{OwnerSurrender}~ (i) ~+~ 0.269 ~\cdot~ I_{Stray}~ (i)$$
...
$$I_{\mbox{Died}}(i) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~ith~\mbox{observation is Died},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$

$$I_{\mbox{Euthanized}}(i) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~ith~\mbox{observation is Euthanized},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$

$$I_{\mbox{Foster}}(i) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~ith~\mbox{observation is Foster},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$

$$I_{\mbox{ReturnedToOwner}}(i) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~ith~\mbox{observation is ReturnedToOwner},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$

$$I_{\mbox{OwnerSurrender}}(i) = \left\{
\begin{array}{ll}
1~~\mbox{if position of}~ith~\mbox{observation is OwnerSurrender},\\
0~~\mbox{Otherwise}.\\
\end{array}
\right.$$
...
# Conclusions & Extended and Further work {#sec:Conc}
Based on the above analysis, the negative binomial distribution model is currently selected as the prediction model. Because it has lower deviation than the basic Poisson distribution model and quasi-Poisson distribution model. This is also a better model that can be selected on the basis of GLM.

Data and features determine the upper limit of machine learning, and models and algorithms are approaching this upper limit. So first of all, in the future work, based on the existing analysis, we will do further feature engineering on the data. Improve the online model fitting effect.

Secondly, the accuracy of the model can be improved by expanding the data set, making the model closer to the truth.

Finally, this article has less research on whether there is zero inflation in data, and future work can move closer to this aspect. And try some other models for fitting, such as zero inflated count model, zero-inflated poisson and zero-inflated negative binomial.
